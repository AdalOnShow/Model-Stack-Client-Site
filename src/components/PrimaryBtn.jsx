...
const PrimaryBtn = ({ children, submit, onClick, loader }) => {
  return (
    <button
      onClick={onClick}
      type={submit ? "submit" : "button"}
      className="relative cursor-pointer flex items-center px-8 py-3 overflow-hidden font-medium transition-all bg-indigo-500 rounded-md group"
    >
      <span
      ...
      ></span>
      <span
        className="relative w-full text-center text-base text-white transition-colors duration-200 ease-in-out group-hover:text-white"
      >
        {loader ? (
